...
import { Plus, Save, Trash2, ArrowLeft, Upload, FileText, MessageSquare, ShieldCheck } from 'lucide-react';
                        ...
                        <div className="mb-6">
                            ...
                            <div className="flex gap-4">
                                <button type="button" onClick={() => setGameType('quiz')} className={`flex-1 p-4 border-2 rounded-lg flex items-center gap-3 transition ${gameType === 'quiz' ? 'border-primary bg-blue-50' : 'hover:border-gray-400'}`}>
                                    ...
                                    <div><p className="font-semibold">Kuis Pilihan Ganda</p><p className="text-xs text-gray-500">Format tanya jawab klasik.</p></div>
                                </button>
                                <button type="button" onClick={() => setGameType('story')} className={`flex-1 p-4 border-2 rounded-lg flex items-center gap-3 transition ${gameType === 'story' ? 'border-primary bg-blue-50' : 'hover:border-gray-400'}`}>
                                    ...
                                    <div><p className="font-semibold">Mode Cerita</p><p className="text-xs text-gray-500">Format narasi interaktif.</p></div>
                                </button>
                            </div>
                        </div>

                        <form onSubmit={handleSaveGame} className="space-y-6">
                            {/* --- SEMUA INPUT FORM SEKARANG LENGKAP --- */}
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg" required placeholder="Contoh: Petualangan Keamanan Sosmed" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                                <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg" placeholder="Jelaskan sedikit tentang game Anda..."></textarea>
                            </div>

                            {isAdmin && (
                                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                    <label htmlFor="is_official" className="flex items-center gap-3 cursor-pointer">
                                        <input type="checkbox" id="is_official" checked={isOfficial} onChange={(e) => setIsOfficial(e.target.checked)} className="h-5 w-5 rounded border-gray-300 text-primary focus:ring-primary" />
                                        <div className="flex items-center gap-2">
                                            <ShieldCheck className="text-primary" size={20}/>
                                            <span className="font-semibold text-primary">Jadikan Game Resmi (Khusus Admin)</span>
                                        </div>
                                    </label>
                                </div>
                            )}
