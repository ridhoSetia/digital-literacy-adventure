  ...
  const endGame = useCallback(
    async (isGameOver = false) => {
      ...
      if (!isGameOver) {
        ...
        // Cek apakah ini game baru atau update dari game yang sudah diselesaikan
        ...
        if (existingScore) {
          // Update score yang sudah ada (untuk game yang di-update)
          const newTotalScore = existingScore.score_achieved + score;
          const { data: scoreData, error: scoreError } = await supabase
            .from("scores")
            .update({
              score_achieved: newTotalScore,
              scenario_count: totalScenarios, // Update scenario_count ke jumlah terbaru
            })
          ...
          // Update XP dengan score baru saja (bukan total)
          ...
          // Insert score baru (untuk game yang pertama kali diselesaikan)
          const { data: scoreData, error: scoreError } = await supabase
            .from("scores")
            .insert({
              user_id: user.id,
  ...
  useEffect(() => {
    const fetchGameAndSession = async () => {
      ...
      if (!isReviewMode && user) {
        ...
        if (sessionData) {
          // Ada session in-progress, lanjutkan dari sini
          ...
          // Tidak ada session, cek apakah game pernah diselesaikan
          const { data: scoreData } = await supabase
            .from("scores")
            .select("scenario_count, score_achieved")
            .eq("user_id", user.id)
            .eq("game_id", gameData.id)
            .single();

          if (scoreData && scoreData.scenario_count > 0) {
            // Game pernah diselesaikan, cek apakah ada konten baru
            const completedCount = scoreData.scenario_count;
            const totalCount = gameData.scenarios.length;

            if (totalCount > completedCount) {
              // Ada penambahan scenarios baru! Skip yang sudah diselesaikan
              setCurrentScenarioIndex(completedCount);
              setScore(scoreData.score_achieved); // Lanjutkan dari score sebelumnya
              toast.success(
