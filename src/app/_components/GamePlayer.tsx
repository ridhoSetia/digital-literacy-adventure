...
const Timer = ({ duration, onTimeUp }: { duration: number, onTimeUp: () => void }) => {
  ...
  return <div className="font-mono text-lg font-semibold text-gray-700">Waktu: {timeLeft}s</div>;
};
...
export default function GamePlayer({ gameCode }: { gameCode: string }) {
    ...
    useEffect(() => {
        ...
        if (user) { fetchGameAndCheckCompletion(); }
        else if (user === null) { toast.error("Anda harus login untuk bermain."); router.push('/'); }
    }, [gameCode, user, router, supabase]);
