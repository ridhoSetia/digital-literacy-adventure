...
export default function ProfilePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const gamesPromise = supabase
      .from("games")
      .select("id, title, game_code, cover_image_url, play_count")
        ...
        <div className="bg-slate-900/50 backdrop-blur-sm border border-violet-700 rounded-xl shadow-lg p-8">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            {isEditing ? (
              <form onSubmit={handleUpdateUsername} className="w-full max-w-sm">
                ...
                <div className="flex gap-2 mt-4">
                  <button
                    ...
                    className="flex-1 bg-slate-700 py-2 rounded-lg text-sm font-semibold hover:bg-slate-600"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="flex-1 bg-violet-600 text-white py-2 rounded-lg disabled:bg-gray-400 flex items-center justify-center gap-2 text-sm font-semibold hover:bg-violet-700"
                  >
              ...
              <>
                <div className="flex items-center gap-2">
                  <h1 className="text-3xl font-bold text-gray-100 font-display tracking-wider">
                    {profile.username}
                  </h1>
                  <button
            ...
            <div className="mt-8 w-full border-t border-slate-700 pt-6">
              <h2 className="text-lg font-semibold text-gray-300 mb-4 font-display tracking-wide">
                Statistik Pemain
              </h2>
              <div className="bg-slate-800/50 border border-blue-500 p-4 rounded-lg">
                ...
                <p className="text-4xl font-bold text-cyan-300 font-pixel pt-4">
                  {profile.total_xp}
                </p>
              </div>
            </div>
        ...
        <div className="bg-slate-900/50 backdrop-blur-sm border border-violet-700 rounded-xl shadow-lg p-8">
          ...
          {createdGames.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-lg text-gray-100 truncate">
                        {game.title}
                      </p>
                      <p className="text-sm text-gray-400">
                        {game.play_count} kali dimainkan
                      </p>
                    </div>
                  </div>
            ...
            <div className="text-center text-gray-500 p-8 border-2 border-dashed border-indigo-700 rounded-lg">
              ...
              <Link
                href="/create"
                className="text-yellow-400 font-semibold hover:text-yellow-300 mt-2 inline-block"
              >
                Ayo buat sekarang!
              </Link>
            </div>
