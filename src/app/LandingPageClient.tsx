    ...
    <>
      ...
      <div className="relative z-10">
        ...
        <div className="relative min-h-screen flex flex-col items-center justify-center text-center p-6 overflow-hidden">
          ...
          <FontAwesomeIcon
            icon={faGamepad}
            className="text-[120px] text-indigo-400/70 absolute top-1/2 left-[15%] transform -translate-y-1/2 -translate-x-1/2 -rotate-12 hidden lg:block"
          />
          <FontAwesomeIcon
            icon={faShieldHalved}
            className="text-6xl text-red-500 absolute top-[55%] left-[22%] transform -rotate-12 hidden lg:block"
          />
          <FontAwesomeIcon
            icon={faGamepad}
            className="text-[120px] text-indigo-400/70 absolute top-1/2 right-[15%] transform -translate-y-1/2 translate-x-1/2 rotate-12 hidden lg:block"
          />
          <FontAwesomeIcon
            icon={faHeadphones}
            className="text-6xl text-red-500 absolute top-[60%] right-[20%] transform rotate-12 hidden lg:block"
          />
          ...
          <div className="relative">
            ...
            <FontAwesomeIcon
              icon={faBolt}
              className="text-5xl text-yellow-400 absolute -top-4 -left-8 transform -rotate-12 drop-shadow-md"
            />
            <FontAwesomeIcon
              icon={faBolt}
              className="text-5xl text-yellow-400 absolute -bottom-4 -right-8 transform rotate-[165deg] drop-shadow-md"
            />
            <FontAwesomeIcon
              icon={faBolt}
              className="text-4xl text-yellow-400 absolute top-0 -right-8 transform rotate-12 drop-shadow-md"
            />
          </div>

          <div className="mt-12">
            <h2 className="font-bold text-xl text-gray-200">
              Jelajahi dunia digital dengan aman!
            </h2>
            <p className="mt-2 max-w-lg text-gray-400 text-sm">
              Belajar literasi digital melalui game interaktif yang seru.
            </p>
            <div className="mt-8 flex items-center justify-center gap-6">
              <Link
                href="/create"
                className="btn-pixel bg-violet-600"
                style={{ "--shadow-color": "#4c1d95" } as React.CSSProperties}
              >
                Buat Game
              </Link>
              <button
                onClick={handlePlay}
                className="btn-pixel bg-green-600"
                style={{ "--shadow-color": "#166534" } as React.CSSProperties}
              >
                Start
              </button>
            </div>
        ...
        <div className="container mx-auto px-6">
          ...
          <section className="text-center py-16 bg-transparent relative">
            <h2 className="text-3xl font-bold font-display tracking-wider">
              Bergabunglah Dengan Para Explorer Lain!
            </h2>
            <p className="text-7xl font-bold font-pixel text-yellow-400 mt-4 drop-shadow-lg">
              {totalPlays.toLocaleString("id-ID")}
            </p>
            <p className="text-gray-400 mt-2 font-bold tracking-widest">
